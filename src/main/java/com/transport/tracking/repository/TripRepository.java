...
public interface TripRepository extends CrudRepository<Trip, BigDecimal> {
   ...
   public Trip findLatestDepartSites(@Param("veh") String veh,@Param("date") Date date);
    ...
    @Query("select DISTINCT new com.transport.tracking.response.ResultTripVO(c.code,CURRENT_TIMESTAMP) from  Trip c where c.site = ?1 and (c.docdate between ?2 AND ?3)")
